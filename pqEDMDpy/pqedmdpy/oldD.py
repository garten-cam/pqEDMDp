...
from sympy import Matrix, lambdify, linear_eq_to_matrix, linsolve, symbols


class olddecomposition(object):
    def __init__(
        self,
        observable,
        xtr,
    ...
    def A(self):
        ...
        if not self.observable.nU:
            ...
            a = np.transpose(
                self.U[: -self.observable.nU - 1, : -self.observable.nU - 1]
            )
        return a
    ...
    def C(self):
        ...
        Az, _ = linear_eq_to_matrix(
            list(sol)[0], [symbols("1"), *symbols(f"z:{self.observable.nSV}")]
        )
        # Remove the constant term
        Az = Az[1:, :]
        # Add the constant term in case b = number +- z
        Az[:, 0] = Matrix(
            list(linsolve((b[1:]), symbols(f"z:{self.observable.nSV}")))
        ).T
        # create the c matrix
        c = np.zeros((self.observable.nSV, self.observable.pq_matrix.shape[1]))
    ...
    def xy_eval(obs, xtr):
        ...
        return np.concatenate(
            (
                np.ones((xtr.shape[0], 1)) * obs.r_trx[0][0],
    ...
    def evol_function(self):
        ...
        poly_prod = Matrix(
            np.matmul(
                np.matmul(self.observable.poly_prod, self.observable.r_trx), self.U
            )
        )
        # lambdify...
    ...
    def predict(self, x0, n_points, u=None):
        ...
        # preallocate
        pred = [
            {"sv": np.zeros((n_points[i], self.observable.nSV))} for i in range(len(x0))
        ]
        ...
        for sample in range(len(x0)):
            for step in range(1, n_points[sample]):
                if u is None:
                    ...
                    xprev = np.concatenate(
                        (pred[sample]["sv"][step - 1, :], u[sample][step - 1, :])
                    )
...
class maxLikedecomposition(decomposition):
    ...
    def _set_U(self, xtr, ytr):
        ...
        for u_column in range(1, n_obs):
            ...
            # one parameter
            obj_f = partial(
                maxLikedecomposition.cost_function,
                Q=self.Q,
            ...
            u[:, u_column] = minimize(
                obj_f,  # New objective function f(u_col)
    ...
    def cost_function(u_col, Q, sigma, x_eval, y_eval):
        j = np.sum(
            (y_eval - np.array(np.matmul(x_eval, u_col))) ** 2
            / (sigma**2 + np.matmul(np.matmul(u_col, Q), np.atleast_2d(u_col).T))
        )
        return j
