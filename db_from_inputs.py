...
class dbImport:
    ...
    def concatenateDBs(self):
        ...
        kiln_dataframe = kiln_dataframe.merge(kiln_dfs['data_10s'], on='timestamp', how='left')
    ...
    def importDBs(self):
        ...
        # Get the transfos
        data_transfos = dbImport.import_transfos(self.path_transfos,self.start_date, self.end_date)
    ...
    def import_nseconds(path_to_db, table, start_date, end_date, features):
        ...
        db_cols = pd.read_sql_query(f'SELECT name FROM PRAGMA_TABLE_INFO({table})',connection)
        # 3. Match the columns that fir the list of features to include
        db_import_cols = db_cols.name[db_cols.name.str.contains("|".join(features))]
        ...
        db_nseconds = pd.read_sql_query(f'SELECT "{feature_str}" FROM {table} WHERE ("timestamp" BETWEEN "{start_date}" AND "{end_date}") ORDER BY "TIMESTAMP"',connection)
        return db_nseconds
