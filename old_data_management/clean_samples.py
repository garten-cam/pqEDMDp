...
class cleanSamples:
    ...
    def perShaftData(self, rough_samples=idb.dbImport().akima_kiln_samples(), shaft=1):
        ...
        cycle_index_shaft = np.concatenate((np.array([0]), np.nonzero(
            rough_samples['Cycle - Number'].diff().to_numpy(na_value=0))[0], np.array([len(rough_samples)+1])))
        # 3.2. Match the state and input names with the keys in the df
        state_match = rough_samples.keys(
        )[rough_samples.keys().str.contains("|".join(self.states))]
        input_match = rough_samples.keys(
        )[rough_samples.keys().str.contains("|".join(self.inputs))]
        # 3.3. Now....... loop! take only the part of the cycle when the burning digit is guan
        # 3.3.1. match the burning digit
        burn = rough_samples.keys(
        )[rough_samples.keys().str.contains('burning_dig')]
        ...
        for sample in range(cycle_index_shaft.size - 1):
            ...
            burn_index = rough_samples[burn].iloc[cycle_index_shaft[sample]
                :cycle_index_shaft[sample+1]].to_numpy().ravel().astype(bool)
            samples[sample]['sv'] = (rough_samples[state_match].iloc[cycle_index_shaft[sample]
                                     :cycle_index_shaft[sample+1]].loc[burn_index]).to_numpy()
            samples[sample]['u'] = (rough_samples[input_match].iloc[cycle_index_shaft[sample]
                                    :cycle_index_shaft[sample+1]].loc[burn_index]).to_numpy()
            samples[sample]['t'] = rough_samples['timestamp'].iloc[cycle_index_shaft[sample]
                :cycle_index_shaft[sample+1]].loc[burn_index]
    ...
    def getDigits(rough_samples, cycle_index):
        ...
        digits = dict()
        digits['running_digit'] = cleanSamples.checkRunningDigit(
            rough_samples, cycle_index)
        # 2. This is tricky... not-not-double burning, returns True if the sample
        # should be included
        digits['inclusive_burning_digit'] = cleanSamples.checkDoubleBurning(
            rough_samples, cycle_index)
        # Ok, these are the two things included in matlab.
        selection_digit = np.logical_and(
            digits['running_digit'], digits['inclusive_burning_digit'])
