...
for ic, expi, ui, t_fi, n_pi in zip(ics, exp, u, tfin, n_points):
    ...
    expi["y_det"] = sol
    expi["y"] = sol + np.random.normal(0, 0.001, sol.shape)
    expi["t"] = t

# Create the pqEDMD object
pqe = pqEDMDp(
    p=[2, 3, 4, 5],          # Sweep over there 3 values of max order p
    q=[0.2, 0.5, 1, 1.5, 2],   # Sweep over these q-quasi-norms
    obs=pqo.legendreObs,  # Use Legendre observables orthogonal [-inf, inf]
    dyn_dcp=lambda obs, sys: sid.sidDecomposition(5, 7, obs, sys))
